...
export default function ProviderCarousel(props: {
  ...
  const connectProviderIdp = async (
    ...
    if (providerIdpData == undefined) {
      ...
      try {
        ...
        const apiResponse = await fetch(`/api/ssr/providers/${id}/idps`, {
          method: "POST",
          headers: {
  ...
  /* Region */
  const [showProviderRegionModal, setShowProviderRegionModal] = useState(false);
  ...
  const editProvider = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    // Prevent the default form submission (page reload)
    e.preventDefault();
    ...
    <>
      {provider.tester_name != "" && (
        <div className="flex items-center">
          Assigned to:
          <UserIcon className="mr-1 ml-4 size-4" />
                ...
                <CarouselPanel>
                  <div className="mt-12 mb-8 w-full">
                    <div className="flex flex-col items-center justify-between md:flex-row">
                      <div className="mb-2 flex w-full flex-col lg:w-7/10">
      ...
      {provider.status == 1 && (
        <>
          <div className="bg-gray/10 mt-12 mb-8 flex w-full flex-row rounded-3xl p-8 text-sm">
            {/* <ClockIcon className='w-6 mr-2 opacity-30' /> */}
            <div>
              <h3 className="mb-4 flex">The provider has been submitted</h3>
              <div>
                The provider has been successfully submitted and is now awaiting
                the next step. The SLA Manager must connect the SLA in order to
        ...
        <>
          {!userRoles.includes("site-tester") && (
            <div className="bg-gray/10 mt-12 mb-8 flex w-full flex-row rounded-3xl p-8 text-sm">
              {/* <ClockIcon className='w-6 mr-2 opacity-30' /> */}
              <div>
                <h3 className="mb-4 flex">The provider is ready for testing</h3>
                <div>
                  The provider setup has been successfully completed and is now
                  ready for validation. Both manual and automated tests will be
                  conducted by the site tester to ensure everything is working
                  as expected.
                  <br />
      ...
      >
        <p>
          ...
          This action is irreversible and you will not be able to retrieve your
          provider anymore.
        </p>
      </ConfirmModal>
      ...
      >
        <p>
          ...
          irreversible and you will not be able to retrieve your IDP anymore.
        </p>
      </ConfirmModal>
      ...
      >
        <p>
          Are you sure you want to delete the <b>{providerRegionData?.name}</b>{" "}
          region? This action is irreversible and you will not be able to
          retrieve your region anymore.
        </p>
      </ConfirmModal>
      ...
      >
        <p>
          Are you sure you want to delete the <b>{providerProjectData?.name}</b>{" "}
          project? This action is irreversible and you will not be able to
        ...
        {providerIdps.length == 0 ? (
          ...
          <ul className="mt-6 w-full">
            {providerIdps.map(row => (
              <li
                key={row.idp_id}
                className="box-sm flex w-full flex-row items-start justify-between lg:items-center"
              >
                <div className="flex w-9/10 flex-col truncate lg:flex-row">
                  <div className="text-md truncate font-bold lg:w-1/2">
                    {row.overrides.name}
                  </div>
                </div>
                {(userRoles.includes("admin") ||
                  ...
                  <div className="flex flex-col">
                    <Options>
                      ...
                      {provider.status != 2 ||
                        (provider.status == 2 && providerIdps.length > 1 && (
                          <Option
                            data-danger={true}
                            onClick={() => {
                              setProviderIdpData(row);
                              setShowProviderIdpDeleteModal(true);
                            }}
                          >
                            <div className="flex items-center">
                              <TrashIcon className="size-4" />
        ...
        >
          <ModalBody>
            <Form onSubmit={connectProviderIdp}>
              {idps.length == 0 ? (
                <p>
                  No identity providers available. Please ask your{" "}
                  <b>SLA Manager</b> to add at least one identity provider.
                </p>
              ) : (
                <ProviderIdpForm />
              )}
        ...
        {providerRegions.length == 0 ? (
          ...
          <ul className="mt-6 w-full">
            {providerRegions.map(row => (
              <li
                key={row.id}
                className="box-sm flex w-full flex-row items-start justify-between"
              >
                <div className="mr-2 flex w-9/10 flex-col">
                  <div className="text-md truncate font-bold">{row.name}</div>
                ...
                {(userRoles.includes("admin") ||
                  ...
                  <div className="flex flex-col">
                    <Options>
                      ...
                      {provider.status != 2 ||
                        (provider.status == 2 && providerRegions.length > 1 && (
                          <Option
                            data-danger={true}
                            onClick={() => {
                              setProviderRegionData(row);
                              setShowProviderRegionDeleteModal(true);
                            }}
                          >
                            <div className="flex items-center">
                              <TrashIcon className="size-4" />
                ...
                <div className="mr-2 flex w-9/10 flex-col">
                  <div className="flex flex-col items-start lg:flex-row lg:justify-between">
                    <div className="w-9/10">
                      <div className="text-md truncate font-bold">
                        {row.name}
                      </div>
                      <div className="text-md truncate">{row.description}</div>
                    </div>
                    {row.is_root ? (
                      <div className="mb-4 flex items-center text-xs font-bold uppercase opacity-80 lg:mb-0">
                          ...
                          providerProjects.length > 1 && (
                            <Option
                              ...
                              onClick={() => {
                                ...
                                setShowProviderProjectDeleteModal(true);
                              }}
                            >
                              <div className="flex items-center">
                                <TrashIcon className="size-4" />
