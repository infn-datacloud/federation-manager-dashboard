  ...
  return (
    <>
      ...
      <h1 className="mb-6 flex items-center">
        <DocumentDuplicateIcon className="mr-4 size-10" />
        SLA
      </h1>
  ...
  return (
    <>
      ...
      <Link href={sla.url} target="_blank">
        <div className="mt-2 underline opacity-50">{sla.url}</div>
      </Link>
      <div className="mt-4 text-justify">{sla.description}</div>
      <div className="mt-4 flex flex-col">
        <div className="flex items-center">
          <CalendarDaysIcon className="mr-2 inline size-5" />
...
function getRemainingTime(endDate: string | Date): string | JSX.Element {
  ...
  if (diff <= 0)
    return (
      <div className="bg-danger/80 mt-4 rounded-md px-6 py-4 text-white">
        <div className="mb-1 flex items-center">
          <ExclamationTriangleIcon className="mr-1 size-4" />
          <div className="font-bold">SLA Expired</div>
        </div>
        <div>
          This SLA can no longer be associated with any project, as it expired
          on {end.toLocaleDateString("it-IT")}.
        </div>
      </div>
    );
