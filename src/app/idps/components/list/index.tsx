...
export default function List(props: Readonly<ListProps>) {
  ...
  const listItems = items?.map(item => (
    ...
    >
      ...
      <div className="truncate text-sm font-semibold opacity-80">
        {item.endpoint}
      </div>
      {item.description && (
        <div className="mt-4 line-clamp-2 text-sm opacity-80">
          {item.description}
        </div>
      )}
    </Link>
  ...
  if (!items || items.length === 0) {
    return (
      <div className="flex flex-col items-center pt-24 opacity-80">
        ...
        <h2 className="py-4 text-center">No IDPs found</h2>
